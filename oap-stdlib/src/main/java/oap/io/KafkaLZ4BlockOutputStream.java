...
public final class KafkaLZ4BlockOutputStream extends FilterOutputStream {
    ...
    public void write( byte[] b, int off, int len ) throws IOException {
        ...
        while( length > bufferRemainingLength ) {
            // fill remaining space in buffer
            System.arraycopy( b, offset, buffer, bufferOffset, bufferRemainingLength );
            bufferOffset = maxBlockSize;
            writeBlock();
            // compute new offset and length
            offset += bufferRemainingLength;
            length -= bufferRemainingLength;
